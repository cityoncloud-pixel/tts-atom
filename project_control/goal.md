...
> **GGS 导出**（iteration 1，评审 `PASS`）  
> 编译自：`project_control/.ggs/idea.md`  
> 假设记录：`project_control/.ggs/assumptions.md`  
> 评审：`project_control/.ggs/goal.review.json`
...
| 超长文本 | 返回 `TTS_TEXT_TOO_LONG`（默认拒绝，不静默截断） |
...
| 数学表达式分句 | 避免在数字与运算符中间切断；MVP 以标点为主 |
| 缓存一致性 | key 必须包含语气与 format 参数 |
| BFF 超时 | `tts-atom` 返回明确错误与 `timing`；BFF 侧超时不在本仓库实现 |
| 权限 | 本地服务，无认证；不面向公网 |
...
1. MeloTTS 中文童声音色需实机试听，可能需 Phase 2 调参或云端豆包作表演音。
2. Windows 上 MeloTTS 依赖安装复杂度未知，可能影响 `doctor` 首次通过率。
3. 本地 TTS 音质 vs 云端：MVP 以稳定与低延迟为先，不承诺配音级音质。
4. Launcher 托管未在本阶段实现。
5. 详见 `assumptions.md` A-0001～A-0007。
...
## Execution Handoff（GAEH）

**路由**：`spec_first`（Normal）— 先 Mock 全链路绿，再接入 MeloTTS。

**种子任务**：见 `goal.review.json` → `handoff.seed_tasks`。

**验证计划**：`pytest` → CLI synth + cache → HTTP healthz/synthesize → split-synthesize → doctor。
